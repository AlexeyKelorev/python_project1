...
"""
full_amount = {}
with open('catalogue.txt', 'r') as content:

    for item in content:
        item = item.replace("(л)", "").replace("(лаб)", "").replace("(пр)", "").replace("—", "0")
        subject, amount1, amount2, amount3 = item.split()
        full_amount[subject] = int(amount1)+int(amount2)+int(amount3)
    print(full_amount)
